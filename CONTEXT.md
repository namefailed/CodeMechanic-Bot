...
## The 10-Agent Orchestration Architecture
The system relies on a central Orchestrator (using an Event Bus pattern) to coordinate 10 specialized agents:
...
8. **Static Analyzer (Zero-Day Researcher)**: Continuously scans popular repos with Semgrep, Trivy, and Gitleaks for vulnerabilities and leaked secrets (requires Docker).
9. **PR Maintainer**: Watches the bot's open PRs for new maintainer comments and routes that feedback back to the PR Engineer.
10. **Review Tracker**: Detects formal "changes requested" reviews on the bot's PRs and triggers an automated revision.
...
- **Scam Repositories**: A massive percentage of bounties are fake. Maintain a blacklist in `config.yaml` under `governance.blacklist` (e.g., `SecureBananaLabs`, `ClankerNation`).
